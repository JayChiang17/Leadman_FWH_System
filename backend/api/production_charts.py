...
async def module_prod(
    ...
    try:
        ...
        if period == "daily":
            ...
            """
            rows = safe_db_execute(db, sql, (range_start, range_end))
            prod = [
                dict(
    ...
    try:
        ...
        try:
            for r in safe_db_execute(mod_db, """
                SELECT DATE(ts) d, COUNT(*) tot, SUM(kind='A') a, SUM(kind='B') b
                FROM scans WHERE ts >= ? AND ts < ? GROUP BY d
            """, (range_start, range_end)):
...
async def hourly_dist(
    ...
    try:
        ...
        hourly_data = {}
        try:
            for row in safe_db_execute(db, sql, (range_start, range_end)):
                hour = int(row["hr"])
...
async def get_data_summary(current_user: dict = Depends(get_current_user)):
    try:
        ...
        try:
            ...
            recent_row = db.execute("SELECT COUNT(*) count FROM scans WHERE ts >= ? AND ts < ?", (range_start, range_end)).fetchone()
    ...
    try:
        ...
        status_breakdown = db.execute("""
            SELECT 
                ...
                COUNT(*) count,
                ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM scans WHERE ts >= ? AND ts < ?), 2) percentage
            FROM scans
            WHERE ts >= ? AND ts < ?
            GROUP BY UPPER(status)
            ORDER BY count DESC
        """, (range_start, range_end, range_start, range_end)).fetchall()
