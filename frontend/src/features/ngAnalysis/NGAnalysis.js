...
function SHAPCard() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await api.get("ml/shap/summary");
      setData(res.data);
    } catch (e) {
      setError("SHAP data unavailable — run predictions first");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-stroke-subtle flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Zap size={15} className="text-amber-500" />
          <div>
            <p className="text-[10px] uppercase tracking-[0.28em] text-stone-400 font-semibold mb-0.5">SHAP</p>
            <h3 className="text-ink-primary font-semibold text-base leading-tight">Feature Importance</h3>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {data?.sample_count > 0 && (
            <span className="text-[11px] text-stone-400">{data.sample_count.toLocaleString()} predictions</span>
          )}
          <button
            onClick={load}
            disabled={loading}
            className="p-1.5 rounded-lg border border-stroke text-ink-muted hover:bg-surface-base disabled:opacity-40 transition-colors"
          >
      ...
      <div className="px-5 py-4">
        {loading && !features.length && (
          <div className="space-y-2.5">
            {[1,2,3,4,5].map(i => (
              <div key={i} className="h-5 bg-surface-raised rounded animate-pulse" style={{ width: `${85 - i * 10}%` }} />
            ))}
          </div>
        )}

        {!loading && error && (
          <p className="text-xs text-stone-400 text-center py-4">{error}</p>
        )}

        {!error && features.length > 0 && (
          <div className="space-y-2">
            {features.map((f, i) => {
              const barPct = maxShap > 0 ? (f.mean_abs_shap / maxShap) * 100 : 0;
              const isTop3 = i < 3;
              return (
                <div key={f.name} className="group">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-xs font-medium truncate max-w-[65%] ${isTop3 ? "text-ink-primary" : "text-ink-muted"}`}>
                      {f.label}
                    </span>
                    <span className={`text-[11px] font-mono tabular-nums ${isTop3 ? "text-amber-400 font-semibold" : "text-stone-400"}`}>
                      {f.mean_abs_shap.toFixed(4)}
                    </span>
                  </div>
                  <div className="relative h-2 bg-surface-raised rounded-sm overflow-hidden">
                    <div
        ...
        {!loading && !error && features.length > 0 && (
          <p className="text-[10px] text-stone-400 mt-4 text-center">
            Mean |SHAP| — higher = stronger influence on NG risk
          </p>
        )}
      </div>
    </div>
...
function ParetoBar({ pct, color }) {
  return (
    <div className="relative w-full h-2.5 bg-surface-raised rounded-sm overflow-hidden">
      <div
...
function ClusterRow({ cluster, rank, cumPct, isActive, onClick, searchTerm }) {
  ...
  const rankBg =
    rank === 0 ? "bg-signal-error/15 text-red-400"
    : rank === 1 ? "bg-signal-warn/15 text-amber-400"
    : rank <= 3  ? "bg-signal-warn/10 text-amber-400"
  ...
  const highlighted = useMemo(() => {
    ...
    return (
      <>
        {label.slice(0, idx)}
        <mark className="bg-amber-200 text-amber-300 rounded-sm px-0.5">
  ...
  return (
    <button
      className={`w-full text-left border rounded-lg overflow-hidden bg-surface-panel shadow-sm transition-colors group
        ${isActive
          ? "border-teal-400 ring-2 ring-teal-200 shadow-md"
          : "border-stroke hover:border-stroke hover:shadow-md"
        }`}
      onClick={onClick}
    >
      <div className="px-4 py-3">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div className="shrink-0 flex items-center gap-3 text-right">
            <div>
              <div className="text-lg font-black font-mono text-ink-primary leading-none">{cluster.count}</div>
              <div className="text-[10px] text-stone-400 uppercase tracking-wide">cases</div>
            </div>
            <div className="w-16">
              <div className="text-sm font-bold text-ink-secondary leading-none">{cluster.pct}%</div>
              <div className="text-[10px] text-stone-400 leading-none mt-0.5">cum {cumPct}%</div>
            </div>
            <ChevronRight
              size={14}
              className={`transition-colors ${isActive ? "text-teal-500" : "text-stone-300 group-hover:text-ink-muted"}`}
            />
          </div>
        </div>
...
function NG3DPanel() {
  ...
  if (loading) {
    return (
      <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm flex items-center justify-center" style={{ height: 480 }}>
        <div className="text-center text-stone-400">
          <div className="animate-spin w-8 h-8 border-2 border-stroke border-t-teal-500 rounded-full mx-auto mb-2" />
  ...
  if (!points.length || !clusters.length) {
    return (
      <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm flex flex-col items-center justify-center py-24 text-center px-8">
        <Box size={44} className="text-ink-secondary mb-4" />
        <p className="text-ink-muted font-semibold">No 3D data yet</p>
  ...
  return (
    <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-stroke-subtle">
        ...
        <h3 className="text-ink-primary font-semibold text-base leading-tight">NG Cluster × Date × Count</h3>
      ...
      <Plot3D
        ...
        layout={{
          ...
          scene: {
            ...
            xaxis: { ...PLOTLY_DARK_LAYOUT.scene.xaxis, title: "Day", tickmode: "array", tickvals: dates.map((_, i) => i), ticktext: dates.map((d) => d.slice(5)) },
            yaxis: { ...PLOTLY_DARK_LAYOUT.scene.yaxis, title: "Cluster", tickmode: "array", tickvals: clusters.map((_, i) => i), ticktext: clusters.map((c) => c.length > 10 ? c.slice(0, 9) + "…" : c) },
            zaxis: { ...PLOTLY_DARK_LAYOUT.scene.zaxis, title: "Count" },
          },
          legend: { ...PLOTLY_DARK_LAYOUT.legend, x: 0, y: 1, font: { ...PLOTLY_DARK_LAYOUT.legend.font, size: 9 } },
...
function RightIdlePanel({ clusters, onSelect, activeId }) {
  if (!clusters?.length) {
    return (
      <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm flex flex-col items-center justify-center py-24 text-center px-8">
        <BarChart2 size={44} className="text-ink-secondary mb-4" />
        <p className="text-ink-muted font-semibold text-base">No cluster data yet</p>
    ...
    <div className="bg-surface-panel border border-stroke rounded-lg shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-stroke-subtle flex items-center justify-between">
        <div>
          ...
          <h3 className="text-ink-primary font-semibold text-base leading-tight">Failure Frequency Distribution</h3>
        </div>
      ...
      <div className="px-5 pb-5 pt-1 flex flex-col gap-2">
        <div className="flex items-center gap-2 text-xs text-amber-400 bg-signal-warn/10 border border-amber-500/30 rounded-lg px-3 py-2">
        ...
        {topCluster && (
          <div className="flex items-center gap-2 text-xs text-red-400 bg-signal-error/10 border border-red-500/30 rounded-lg px-3 py-2">
...
function BottomSheet({ cluster, onClose }) {
  ...
  return (
    <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end" onClick={onClose}>
      ...
      <div
        ref={sheetRef}
        className="relative bg-surface-panel rounded-t-2xl shadow-lg flex flex-col overflow-hidden"
...
export default function NGAnalysis() {
  ...
  return (
    <div className="min-h-screen bg-surface-base pb-20 md:pb-8">

      {/* ── sticky header ──────────────────────────────────────────── */}
      <header className="sticky top-0 z-40 bg-surface-panel border-b border-stroke shadow-sm">
        <div className="px-4 md:px-6 py-3 md:py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-signal-error/15 rounded-lg">
              <BarChart2 className="w-5 h-5 md:w-6 md:h-6 text-red-400" />
            </div>
            <div>
              <h1 className="text-lg md:text-xl font-bold text-ink-primary leading-tight">ATE NG Analysis</h1>
          ...
          <div className="flex items-center gap-2">
            {isAdmin && (
              <button
                ...
                disabled={retraining}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-red-500/30 bg-signal-error/10 text-red-400 text-xs font-semibold hover:bg-signal-error/15 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
            ...
            <button
              onClick={refresh}
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-stroke bg-surface-panel text-ink-secondary text-xs font-semibold hover:bg-surface-base disabled:opacity-50 transition-colors"
            >
        ...
        {showRetrainBanner && retrainMsg && (
          <div className="px-4 md:px-6 pb-3 flex items-center gap-2">
            <div className="flex-1 text-xs text-teal-400 bg-teal-500/10 border border-teal-500/30 rounded-lg px-3 py-2">
              {retrainMsg} — clustering runs in background, results appear in ~10s
            </div>
            <button
              onClick={() => setShowRetrainBanner(false)}
              className="p-1 text-stone-400 hover:text-ink-secondary transition-colors"
            >
              <X size={14} />
            </button>
          </div>
      ...
      <main className="px-4 py-4 md:px-6 md:py-5 space-y-4 md:space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-surface-panel rounded-lg border border-stroke p-3 md:p-4 shadow-sm">
            <p className="text-xs font-semibold text-ink-muted uppercase tracking-wide">Total NG</p>
            <p className="text-2xl md:text-3xl font-bold text-ink-primary mt-1">{totalNG.toLocaleString()}</p>
          </div>
          <div className="bg-surface-panel rounded-lg border border-red-500/30 p-3 md:p-4 shadow-sm">
            <p className="text-xs font-semibold text-red-400 uppercase tracking-wide">Clusters</p>
            <p className="text-2xl md:text-3xl font-bold text-red-400 mt-1">{clusters?.length ?? 0}</p>
          </div>
          <div className="bg-surface-panel rounded-lg border border-amber-500/30 p-3 md:p-4 shadow-sm">
            <p className="text-xs font-semibold text-amber-400 uppercase tracking-wide">Top Issue</p>
            <p className="text-2xl md:text-3xl font-bold text-amber-400 mt-1">{topPct > 0 ? `${topPct}%` : "—"}</p>
          </div>
          <div className="bg-surface-panel rounded-lg border border-stroke p-3 md:p-4 shadow-sm">
            <p className="text-xs font-semibold text-ink-muted uppercase tracking-wide">Coverage</p>
            <p className="text-2xl md:text-3xl font-bold text-ink-secondary mt-1">{totalNG > 0 ? `${coveragePct}%` : "—"}</p>
            {noiseCount > 0 && (
        ...
        {/* ── error state ──────────────────────────────────────────── */}
        {!loading && error && (
          <div className="flex items-center gap-2 text-sm text-red-400 bg-signal-error/10 border border-red-500/30 rounded-lg px-4 py-3">
            ...
            {withCum.length > 3 && (
              <div className="relative mb-3">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-8 py-2 text-sm bg-surface-panel border border-stroke rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400 transition-colors"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-ink-secondary"
                  >
                    <X size={13} />
                  </button>
                )}
            ...
            <div ref={listRef} className="space-y-3">
              ...
              {loading && !clusters?.length && (
                <div className="space-y-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="h-[76px] bg-surface-panel border border-stroke rounded-lg animate-pulse" />
                  ))}
                </div>
              )}

              {!loading && !error && clusters?.length === 0 && (
                <div className="flex flex-col items-center gap-3 py-20 bg-surface-panel border border-stroke rounded-lg">
                  <BarChart2 size={40} className="text-ink-secondary" />
                  <p className="text-base font-semibold text-ink-muted">No clusters yet</p>
              ...
              {search && filtered.length === 0 && withCum.length > 0 && (
                <div className="flex flex-col items-center gap-2 py-12 bg-surface-panel border border-stroke rounded-lg">
                  <Search size={28} className="text-ink-secondary" />
                  <p className="text-sm text-stone-400">No clusters match "{search}"</p>
                  <button onClick={() => setSearch("")} className="text-xs text-teal-400 hover:underline">Clear search</button>
                </div>
              )}
                  ...
                  <>
                    ...
                    {!search && eightyIdx >= 0 && eightyIdx < list.length - 1 && (
                      <div className="flex items-center gap-3 py-1">
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-amber-400 font-semibold bg-signal-warn/10 border border-amber-500/30 px-2.5 py-1 rounded-lg">
                          <TrendingUp size={11} />
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-1 bg-surface-raised rounded-lg p-1 w-fit">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${rightView === "pareto" ? "bg-surface-panel text-ink-primary shadow-sm" : "text-ink-muted hover:text-ink-secondary"}`}
                  >
                    <span className="flex items-center gap-1.5"><BarChart2 size={11} />Pareto</span>
                  </button>
                  <button
                    onClick={() => setRightView("3d")}
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${rightView === "3d" ? "bg-surface-panel text-ink-primary shadow-sm" : "text-ink-muted hover:text-ink-secondary"}`}
                  >
                    <span className="flex items-center gap-1.5"><Box size={11} />3D Scatter</span>
                  </button>
                  <button
                    onClick={() => setRightView("shap")}
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${rightView === "shap" ? "bg-surface-panel text-ink-primary shadow-sm" : "text-ink-muted hover:text-ink-secondary"}`}
                  >
                    <span className="flex items-center gap-1.5"><Zap size={11} />SHAP</span>
                  </button>
                </div>
                {rightView === "pareto" ? (
                  <RightIdlePanel
